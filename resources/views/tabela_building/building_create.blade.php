<x-app-layout>
    ...
    <div class="max-w-2xl mx-auto py-12">
        ...
        @if (session()->has('message'))
            <div class="bg-green-100 text-green-700 px-8 py-8 rounded mb-6">
                {{ session()->get('message') }}
            </div>
        @endif
        ...
        <form action="{{ route('buildings.store') }}" method="post"
            class="bg-white dark:bg-gray-800 shadow-md rounded px-3 pt-3 pb-3 mb-3">
            @csrf

            <div class="mb-4">
                <label for="nome_predio" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2"
                    for="nome_predio">Nome do Predio</label>
                <input type="text" name="nome_predio" id="nome_predio" placeholder="Nome do predio"
            ...
            <div class="mb-4">
                <label for="apartamentos" class="block text-gray-700 dark:text-gray-300 text-sm font-bold mb-2"
                    for="apartamentos">Apartamento</label>
                <input type="number" name="apartamentos" id="apartamentos" placeholder="Apartemento"
                    class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 dark:text-gray-300 leading-tight focus:outline-none focus:shadow-outline">
            </div>
